...
(defn connect! [live-endpoint-url on-msg on-connected & {:keys [transform-response]
                                                         :or {:transform-response identity}
                                                         :as opts}]
  ...
  (when-not @socket
    ...
    (let [sock (js/WebSocket. live-endpoint-url)]
      ...
      (set! (.-onclose sock) (on-dead-connection-fn
                              #(apply connect! live-endpoint-url on-msg on-connected opts)))
